      ...
      {canManageKnowledge && (
        <Card padding="md">
          <div className="mb-3 flex items-center justify-between gap-3">
            <div>
              <h2 className="flex items-center gap-2 text-sm font-semibold text-text-primary"><BookOpen className="h-4 w-4 text-primary" />讲义入库记录</h2>
              <p className="mt-1 text-xs text-text-tertiary">新生成且校验通过的专属讲义会自动按所属领域共享；这里处理历史资源和发布异常。</p>
            </div>
          ...
          {publicationRequests.length === 0 ? <p className="py-5 text-center text-sm text-text-tertiary">暂无入库记录</p> : <div className="divide-y divide-border/50">{publicationRequests.map((request) => { const statusLabel = request.status === 'pending' ? '待管理员审核' : request.status === 'waiting_validation' ? '等待校验' : request.status === 'publishing' ? '自动入库中' : request.status === 'published' ? '已入库' : request.status === 'rejected' ? '已驳回' : '发布失败'; const title = String(request.snapshot?.title || `资源 #${request.resourceId}`); return <div key={request.id} className="flex flex-wrap items-center justify-between gap-3 py-3"><div className="min-w-0"><p className="truncate text-sm font-medium text-text-primary">{title}</p><p className="mt-1 text-xs text-text-tertiary">资源 #{request.resourceId} · v{request.resourceVersion} · {new Date(request.submittedAt).toLocaleString('zh-CN')}</p>{request.errorMessage && <p className="mt-1 text-xs text-error">{request.errorMessage}</p>}{request.reviewNote && !request.errorMessage && <p className={`mt-1 text-xs ${request.status === 'rejected' ? 'text-error' : 'text-text-tertiary'}`}>{request.reviewNote}</p>}</div><div className="flex items-center gap-2"><Badge variant={request.status === 'published' ? 'success' : request.status === 'rejected' || request.status === 'publish_failed' ? 'error' : 'warning'}>{statusLabel}</Badge>{request.status === 'pending' && <><Button variant="outline" size="sm" disabled={publicationActionId === request.id} onClick={() => void handlePublicationAction(request, 'reject')}>驳回</Button><Button variant="primary" size="sm" disabled={publicationActionId === request.id} onClick={() => void handlePublicationAction(request, 'approve')}>批准</Button></>}{request.status === 'publish_failed' && <Button variant="outline" size="sm" disabled={publicationActionId === request.id} onClick={() => void handlePublicationAction(request, 'retry')}><RefreshCw className="h-3.5 w-3.5" />重试</Button>}</div></div> })}</div>}
        </Card>
      )}
