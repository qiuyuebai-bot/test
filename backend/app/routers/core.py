...
router = APIRouter(prefix="", tags=["核心业务"])
...
def get_blind_area_heatmap(
    ...
    数据格式对齐前端Recharts HeatmapChart需求
    """
    try:
        from app.models import LearnerProfile
...
def get_ability_trend(
    ...
    从答题记录按周聚合平均分，用于前端 AreaChart 展示
    """
    try:
