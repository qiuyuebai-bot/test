...
当前全系统幻觉率为 `3/53 = 5.66%`。分子是已完成证据审查且确认存在幻觉的记录，分母是已完成的证据审查记录；`insufficient_evidence`、知识缺口和未完成审查不进入分子或分母。当前另有 88 条待审查记录和 123 条证据缺口记录，这两个数量可能重叠，不能相加作为样本量。
...
| 当前幻觉率 | 5.66% | `H / E * 100` |
| 待审查 `P` | 88 | 尚未形成最终裁决，不计入分母 |
| 证据缺口 `G` | 123 | 没有足够证据，不计入分子或分母 |
